...
function AthleteDetails({ userId, onBack }) {
    ...
    const fetchAthleteData = async () => {
      ...
      try {
        ...
        const [
          ...
          fetch(`http://localhost:3001/documents/${userId}`),
          fetch(`http://localhost:3001/grades-report/${userId}`),
          fetch(`http://localhost:3001/payments-report/${userId}`)
        ]);
  ...
  const handleDeleteDocument = async (docId) => {
    ...
    try {
      const response = await fetch(`http://localhost:3001/documents/${docId}`, { method: 'DELETE' });
  ...
  const handleDeleteGrade = async (gradeId) => {
    ...
    try {
      const response = await fetch(`http://localhost:3001/grades/${gradeId}`, { method: 'DELETE' });
      const data = await response.json();
      if (response.ok) {
        alert(data.message);
  ...
  const handleUpdatePaymentStatus = async (paymentId, newStatus) => {
    try {
      const response = await fetch(`http://localhost:3001/payments/${paymentId}/status`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ status: newStatus }) });
      const data = await response.json();
      if (response.ok) {
            ...
            documents.map(doc => (
              <div key={doc.id} className="list-item">
                ...
                <span className="col-link">
                  <a href={`http://localhost:3001/uploads/${doc.filename}`} target="_blank" rel="noopener noreferrer">
                    Ver Arquivo
                  </a>
                </span>
